...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const router = async () => {
        ...
        if (State.userProfile && State.userProfile.subscription_status === 'Trial Expired') {
            if (pageName !== 'billing') {
                showToast('Quota reached for free trial. Please upgrade to a Starter or Pro plan to continue.', 'error');
    ...
    const initAuthListener = () => {
        dbClient.auth.onAuthStateChange(async (event, session) => {
            // Skip duplicate/concurrent events (TOKEN_REFRESHED, etc.)
            ...
            try {
                ...
                if (session) {
                    ...
                    if (State.userProfile) {
                        ...
                        document.getElementById('sidebar-plan-label').textContent = `${State.userProfile.subscription_plan} Plan`;
                ...
                try {
                    fetch(window.API_BASE_URL + `/api/user/usage?userId=${State.currentUser.id}`)
                        ...
                        .then(data => {
                            ...
                            document.getElementById('trial-days-text').textContent = `${apiCalls} / 40 Calls`;
                            document.getElementById('trial-days-bar').style.width = `${Math.max(0, Math.min(100, (apiCalls / 40) * 100))}%`;
    ...
    const renderCamerasGrid = async () => {
        ...
        State.cameraList.forEach(cam => {
            // If this camera card already exists in the DOM, skip rebuilding it (stream is live)
            const existingCard = grid.querySelector(`[data-cam-id="${cam.id}"]`);
            if (existingCard) {
                // Just update the prompt snippet and status dot if they changed
    ...
    const updateGridSizeClass = () => {
        camerasGridList.classList.remove('grid-1', 'grid-4', 'grid-6', 'grid-9');
    ...
    btnViewCards?.addEventListener('click', () => {
        ...
        camerasGridList.classList.remove('command-center-mode', 'grid-1', 'grid-4', 'grid-6', 'grid-9');
    });
    ...
    if (promptInput && charCountEl) {
        promptInput.addEventListener('input', () => {
            const maxChars = State.userProfile.subscription_plan === 'Free' ? 25 : (State.userProfile.subscription_plan === 'Starter' ? 45 : 55);
    ...
    const openCameraDetailsModal = async (camera) => {
        ...
        // Live-poll detection history every 1.5s so new alerts appear automatically (real-time feel)
        if (detectionHistoryPoller) clearInterval(detectionHistoryPoller);
        detectionHistoryPoller = setInterval(() => {
            if (document.hidden) return;
        ...
        textarea.oninput = () => {
            ...
            autosaveDebounceTimer = setTimeout(async () => {
                ...
                if (!isSaveBtnDisabled) {
                    ...
                    await dbClient.addPrompt({ camera_id: camera.id, user_id: State.currentUser.id, prompt_text: val });
    ...
    const renderBillingDetails = async () => {
        ...
        const { data: subData } = await dbClient.getSubscription(State.currentUser.id);

        // Use subData.plan_name or fallback to State.userProfile or 'Free'
        const plan = (subData && (subData.plan_name || subData.plan)) || State.userProfile.subscription_plan || 'Free';
        const camerasCount = State.cameraList.length;
        ...
        if (subData) {
            ...
            const fallbackStartDate = subData.start_date || State.userProfile.created_at || new Date().toISOString();
    ...
    // Handle Purchase Walkthrough Modal
    const showPurchaseWalkthrough = (oldPlan, newPlan) => {
        const modal = document.getElementById('purchase-walkthrough-modal');
        if (!modal) return;

        // Set Plan Names
        const displayOldPlan = oldPlan === 'Free' ? 'Free Trial' : oldPlan;
        document.getElementById('walkthrough-new-plan-name').textContent = newPlan;
        document.getElementById('walkthrough-old-plan-title').textContent = `${displayOldPlan} Plan`;
        document.getElementById('walkthrough-new-plan-title-col').textContent = `${newPlan} Plan`;

        // Define Features
        const plans = {
            'Free Trial': [
                { name: 'Camera Feeds', val: '1', inc: false },
                { name: 'Custom Prompts', val: '25 Chars', inc: false },
                { name: 'Alert Types', val: 'Web Push', inc: false },
                { name: 'Max Alerts', val: '20 Total', inc: false },
                { name: 'Inference', val: 'Standard', inc: false },
                { name: 'AI Event Search', val: '🔒 Upgrade to unlock', inc: false, isLock: true }
            ],
            'Starter': [
                { name: 'Camera Feeds', val: '3', inc: true },
                { name: 'Custom Prompts', val: '45 Chars', inc: true },
                { name: 'Alert Types', val: 'Web & Email', inc: true },
                { name: 'Max Alerts', val: '60 / Day', inc: true },
                { name: 'Inference', val: 'Priority', inc: true },
                { name: 'AI Event Search', val: 'Unlocked', inc: true }
            ],
            'Pro': [
                { name: 'Camera Feeds', val: '6', inc: true },
                { name: 'Custom Prompts', val: '55 Chars', inc: true },
                { name: 'Alert Types', val: 'Web & Email', inc: true },
                { name: 'Max Alerts', val: '150 / Day', inc: true },
                { name: 'Inference', val: 'Priority', inc: true },
                { name: 'AI Event Search', val: 'Unlocked', inc: true }
            ]
        };

        const oldFeatures = plans[oldPlan] || plans['Free Trial'];
        const newFeatures = plans[newPlan] || plans['Starter'];

        const oldList = document.getElementById('walkthrough-old-features');
        const newList = document.getElementById('walkthrough-new-features');

        oldList.innerHTML = oldFeatures.map(f => `
            <li>
                <i data-lucide="${f.isLock ? 'lock' : (f.inc ? 'check' : 'x')}" style="color: ${f.isLock ? 'var(--text-muted)' : (f.inc ? 'var(--success-color, #10b981)' : 'var(--danger-color, #ef4444)')};"></i>
                <span class="feature-text" style="${f.isLock ? 'color: var(--text-muted);' : ''}">${f.val} ${f.name}</span>
            </li>
        `).join('');

        newList.innerHTML = newFeatures.map(f => `
            <li>
                <i data-lucide="check" style="color: var(--success-color, #10b981);"></i>
                <span class="feature-text"><strong>${f.val}</strong> ${f.name}</span>
            </li>
        `).join('');

        if (typeof lucide !== 'undefined') lucide.createIcons();

        // Show Modal (Step 1 active by default)
        modal.dataset.oldPlan = oldPlan; // Store old plan to check if step 3 should be skipped
        document.getElementById('walkthrough-step-1').style.display = 'block';
        document.getElementById('walkthrough-step-2').style.display = 'none';
        
        const step3 = document.getElementById('walkthrough-step-3');
        if (step3) step3.style.display = 'none';

        const next2Btn = document.getElementById('btn-walkthrough-next-2');
        if (next2Btn) {
            if (oldPlan === 'Starter' || oldPlan === 'Pro') {
                next2Btn.innerHTML = 'Go to Dashboard <i data-lucide="arrow-right" style="margin-left: 8px;"></i>';
            } else {
                next2Btn.innerHTML = 'Next <i data-lucide="arrow-right" style="margin-left: 8px;"></i>';
            }
        }
    ...
    document.getElementById('btn-walkthrough-next-2')?.addEventListener('click', () => {
        ...
        if (lockText) { lockText.textContent = 'Unlocking your semantic search...'; }
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        if (lockText) {
                            lockText.textContent = 'AI Event Search is now fully unlocked and ready!';
                        }
                        if (finishBtn) {
